...
- 퍼징/속성 테스트 적용(공통 규약)
	- 작업: 값 파서에 fuzz/proptest 추가(허용값 이외 거부 속성).
	- 수용 기준: 신규 테스트 PASS.
- 불변 상수 경계 케이스 추가
	- 작업: `?1`/`?0` 외 입력 거부 테스트만 추가. 코드 변화 최소화.
	- 수용 기준: 기존 테스트 유지 + 신규 테스트 통과.

## X-DNS-Prefetch-Control
- 퍼징/속성 테스트 적용(공통 규약)
	- 작업: on/off 파서에 fuzz/proptest 추가.
	- 수용 기준: 신규 테스트 PASS.
- 경계값 트리밍 강화
	- 작업: on/off 외 입력 거부 시 에러 메시지 통일.
	- 수용 기준: 기존 테스트 유지.

## X-Frame-Options
- 퍼징/속성 테스트 적용(공통 규약)
	- 작업: 대소문/공백 혼합 입력 정규화 경로에 fuzz/proptest 추가.
	- 수용 기준: 신규 테스트 PASS.
- 중복값/케이싱 보정
	- 작업: SAMEORIGIN/DENY 대소문 혼합 입력 정규화 경로 재검증, 에러 메시지 통일화.
	- 수용 기준: 기존 테스트 유지.
...
- 퍼징/속성 테스트 적용(공통 규약)
	...
	- 수용 기준: 신규 테스트 PASS.
- 값 강제 로직 단순화
	- 작업: executor에서 무조건 nosniff로 대입하는 경로 유지, 불필요 분기 제거 여부 점검.
	- 수용 기준: 기존 테스트 유지.

## X-Powered-By
- 퍼징/속성 테스트 적용(공통 규약)
	- 작업: 헤더 키 다양한 대소문/공백 변형 제거 경로에 fuzz/proptest 추가(멱등·완전 제거 속성).
	- 수용 기준: 신규 테스트 PASS.

## Normalized Headers 컴포넌트
- 대용량 입력 성능 벤치 확대
	- 작업: 수십만 길이 헤더/수천 멀티값 분할 케이스 벤치 추가.
	- 수용 기준: 벤치 실행 가능, 회귀 없음.

- 속성 테스트 강화
	- 작업: 분할/정규화 멱등성, 케이스 비의존 동치성, 삽입/삭제 결합 속성 추가.
	- 수용 기준: 신규 테스트 PASS.
- 제거 경로 케이스 확장(테스트)
	- 작업: 대소문 혼합/공백 포함 키 제거 케이스 테스트 추가. 코드 변화는 없음.
	- 수용 기준: 기존 테스트 유지 + 신규 테스트 통과.
