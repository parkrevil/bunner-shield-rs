...
- [x] 퍼징/속성 테스트 적용(공통 규약)
	- 작업: enable/disable(`?1`/`?0`) 마커 강제 설정에 대한 proptest 추가(멱등·키 정규화·기존 값 덮어쓰기 확인).
	- 수용 기준: 신규 테스트 PASS.
	- 테스트 구분: 통합 테스트(tests/origin_agent_cluster.rs) + 프롭테스트 완료.
- [x] 불변 상수 경계 케이스 추가
	- 작업: `?1`/`?0` 외 임의 값이 있어도 옵션에 맞게 최종 값이 강제됨을 검증(파서 없음 → 거부 대신 덮어쓰기 테스트로 대체).
	- 수용 기준: 기존 테스트 유지 + 신규 테스트 통과.

## X-DNS-Prefetch-Control
- [ ] 퍼징/속성 테스트 적용(공통 규약)
	- 작업: On/Off 정책에 대해 proptest 추가(기존 값 덮어쓰기·멱등성·키 정규화 확인).
	- 수용 기준: 신규 테스트 PASS.
	- 테스트 구분: 통합 테스트(tests/x_dns_prefetch_control.rs) + 프롭테스트 추가 예정.
- [x] 경계값 트리밍 강화 (N/A)
	- 작업: 문자열 파서 없음 → 에러 메시지 통일 적용 대상 아님. Executor는 정책 값만 설정.
	- 수용 기준: N/A로 표기, 동작 테스트로 대체.

## X-Frame-Options
- [x] 퍼징/속성 테스트 적용(공통 규약)
	- 작업: 대소문/공백 혼합 입력 정규화 경로에 proptest 추가. 정책(DENY/SAMEORIGIN) 각각 멱등성 보장.
	- 수용 기준: 신규 테스트 PASS.
	- 테스트 구분: 통합 테스트(tests/x_frame_options.rs) + 프롭테스트 완료.
- [ ] 중복값/케이싱 보정
...
- [x] 퍼징/속성 테스트 적용(공통 규약)
	- 작업: executor 값 강제 로직의 멱등성 속성 테스트.
	- 수용 기준: 신규 테스트 PASS.
	- 테스트 구분: 통합 테스트 + 프롭테스트(tests/x_content_type_options.rs) 완료.
- [x] 값 강제 로직 단순화
	- 작업: executor에서 무조건 nosniff로 대입(현재 불필요 분기 없음, 단순 로직 충족).
	- 수용 기준: 기존 테스트 유지.

## X-Powered-By
- [x] 퍼징/속성 테스트 적용(공통 규약)
	- 작업: 헤더 키 다양한 대소문/공백 변형 제거 경로에 proptest 추가(멱등·완전 제거 속성).
	- 수용 기준: 신규 테스트 PASS.
	- 테스트 구분: 통합 테스트 + 프롭테스트(tests/x_powered_by.rs) 완료.

## Normalized Headers 컴포넌트
- ✅ (2025-10-21) 대용량 입력 벤치, 속성 테스트, 제거 경로 케이스 확장 모두 완료.
