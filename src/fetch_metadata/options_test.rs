...
#[test]
fn given_default_options_when_created_then_uses_bundled_policy() {
...
#[test]
fn given_custom_navigation_destinations_when_added_then_deduplicates() {
    let options = FetchMetadataOptions::new()
        .navigation_destinations([FetchDestination::Document])
        .add_navigation_destination(FetchDestination::Document)
        .add_navigation_destination(FetchDestination::Empty);

    assert_eq!(
        options.navigation_destinations,
        vec![FetchDestination::Document, FetchDestination::Empty]
    );
}
...
fn given_allow_cross_site_rule_when_added_twice_then_keeps_single_entry() {
    ...
    assert_eq!(options.cross_site_allowances.len(), 1);
    assert_eq!(options.cross_site_allowances[0], rule);
}
